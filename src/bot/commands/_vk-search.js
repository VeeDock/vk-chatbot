 ...
 * @private
 */
const checker = require('../base/modules/messages/url-replacer').check;
...
async function processCommand (type, { id, app, args, options }) {
  ...
  let params = Object.assign({}, { q: query, count }, options.params);

  return await VK.call(options.label + '.search', params)
    .then(response => {
