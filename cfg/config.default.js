...
module.exports = {
  ...
  bot: {
    ...
    // Разрешения, которые будут получены от аккаунта бота при авторизации.
    scope: 'friends,photos,video,audio,docs,groups,status,messages,wall,offline'
  }, 
  ...
  commands: {
    ...
    // commands/vote.js
    vote: {
      duration: 2 // длительность в минутах [2-4]
    }, 
  ...
  path: {
    ...
    modes:    path.resolve(__dirname, '../bot/chat-modes/'), 
    parsers:  path.resolve(__dirname, '../bot/parsers/'), 
    tokens:   path.resolve(__dirname, '../.temp/tokens.json')
  }, 
  ...
  // Callback API & токен сообщества.
  vk_group: {
    confirm: '<confirm_key>', // String
